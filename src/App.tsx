...
function App() {
  ...
  const handleSelectStyle = (style: Style, isMultiSelect: boolean) => {
    if (!activeProject) return;

    const { selectedStyles } = activeProject;
    const isAlreadySelected = selectedStyles.some(s => s.id === style.id);

    let newSelectedStyles: Style[];

    if (isMultiSelect) {
        if (isAlreadySelected) {
            newSelectedStyles = selectedStyles.filter(s => s.id !== style.id);
        } else {
            if (selectedStyles.length < 2) {
                newSelectedStyles = [...selectedStyles, style];
            } else {
                showToast("Você pode mesclar no máximo 2 estilos.", 'normal');
                newSelectedStyles = selectedStyles;
            }
        }
    } else {
        if (isAlreadySelected && selectedStyles.length === 1) {
            newSelectedStyles = [];
        } else {
            newSelectedStyles = [style];
        }
    }
    updateActiveProjectState({ selectedStyles: newSelectedStyles });
  };

  const handleGenerateStyleVariations = async (styleToVary: Style) => {
    if (!activeProject) return;
    const variations = await handleApiCall(
        () => generateStyleVariations(styleToVary),
        `Gerando variações para "${styleToVary.name}"...`
    );
    if (variations) {
        const variationsWithIds: Style[] = variations.map(s => ({ ...s, id: crypto.randomUUID() }));
        updateActiveProjectState({ styleProposals: [...activeProject.styleProposals, ...variationsWithIds] });
        showToast(`3 novas variações de estilo foram adicionadas!`);
    }
  };

  const handleToggleFavoriteStyle = (style: Style) => {
      if (!activeProject) return;
  ...
  const handleConfirmStyleSelection = (mixPercentages?: { [id: string]: number }) => {
    updateActiveProjectState({ step: 3 });
  };

  const handleConfirmCustomStyle = (promptText: string) => {
    ...
    switch (activeProject.step) {
      ...
      case 2:
        return <Step2_StyleSelection 
                  ...
                  onToggleFavorite={handleToggleFavoriteStyle} 
                  onConfirm={handleConfirmStyleSelection}
                  onConfirmCustomStyle={handleConfirmCustomStyle} 
